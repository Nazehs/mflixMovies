  ...
  */
  ...
  static async addComment(movieId, user, comment, date) {
    try {
      ...
      const commentDoc = { name: user.name, email: user.email, date: date, text: comment, movie_id: ObjectId(movieId) }
  ...
  static async mostActiveCommenters() {
    ...
    */
    try {
      ...
      const pipeline = [{ $group: { _id: '$email', count: { $sum: 1 } } }, { $sort: { count: 1 } }, { $limit: 20 }]
